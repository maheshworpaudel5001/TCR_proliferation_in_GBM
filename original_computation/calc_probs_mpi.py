...
def calc_probs_for_every_TCR(kr, x1, x2, maxM):
    if isinstance(kr, np.ndarray) and kr.ndim == 1:
        with ProcessPoolExecutor() as executor:
            results = list(
                tqdm.tqdm(
                    executor.map(
                        calc_probs_for_single_TCR,
                        kr,
                        [x1] * len(kr),
                        [x2] * len(kr),
                        [maxM] * len(kr),
...
if __name__ == "__main__":
    ...
    params_df = pd.read_excel(
        f"{root_dir}/results/results.xlsx",
        engine="openpyxl",
    ...
    if rank == 0:
        output_file = f"/home/gddaslab/mxp140/tcr_project_ultimate/results/probabilities/probs_{patient_id}_for_mean_kr_test.h5"
