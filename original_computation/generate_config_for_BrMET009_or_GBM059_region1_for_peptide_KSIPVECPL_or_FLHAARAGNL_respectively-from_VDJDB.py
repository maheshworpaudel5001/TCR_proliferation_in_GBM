...
from constants import max_kr_vdjdb
...
if __name__ == "__main__":
    ...
    ##################Prepare data###################
    region = 1
    neoantigen_dict = {"BrMET009": "KSIPVECPL", "GBM059": "FLHAARAGNL"}

    full_data = pd.read_csv(
        f"{root_dir}/data/BrMET_and_GBM_data-ERGOII.csv",
        sep=",",
    )
    full_data = full_data.rename(columns={"kr_vdjdb": "kr"})
    max_kr = max_kr_vdjdb

    patient_data = full_data[full_data["Patient"] == f"{patient_id}"]

    filepath = os.path.join(
        f"{root_dir}/data/glioblastoma_data/ERGOII/{patient_id}/ergo2_output_with_regions.csv"
    )
    region_level_data = pd.read_csv(filepath, sep=",", comment="#")
    req_data = region_level_data[region_level_data[f"region{region}"] == "Yes"]
    req_data = pd.merge(
        req_data[["Peptide", "CDR3", "Score_vdjdb"]],
    ...
    patient_params_df = params_df[params_df["Patient"] == f"{patient_id}"]
    patient_params_df_by_region = patient_params_df[
        patient_params_df["Region"] == f"region{region}"
    ]
    x1_value = patient_params_df_by_region["x1_vdjdb"].values[0]
    x2_value = patient_params_df_by_region["x2_vdjdb"].values[0]
    ...
    antigen = neoantigen_dict.get(patient_id)
    if rank == 0:
        print(f"=" * 80)
        print(f"Processing neoantigen {antigen}...\n")
        sys.stdout.flush()  # Flush the output buffer
    ...
    for i, data_chunk in enumerate(chunked_patient_data):
        if rank == 0:
            ...
            sys.stdout.flush()  # Flush the output buffer
        scaled_kr_values = (
            data_chunk["Score_vdjdb"].values * num_of_neoantigens / max_kr
        )
    ...
    if rank == 0:
        probability_file = f"{root_dir}/probs_{patient_id}_region{region}_{antigen}.h5"
        print(
        ...
        with h5py.File(probability_file, "r") as f:
            ...
            config_filename = (
                f"config_{patient_id}_region{region}_from_{antigen}_from_VDJDB.npz"
            )
            np.savez(config_filename, array_2d=configs)
        ...
        print(f"Configuration is saved in\n{config_filename}.")
        sys.stdout.flush()
