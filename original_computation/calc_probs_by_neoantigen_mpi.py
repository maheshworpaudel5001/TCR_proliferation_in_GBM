...
if __name__ == "__main__":
    ...
    region_level_data_dir = f"{root_dir}/data/glioblastoma_data/ERGOII"
    filepath = os.path.join(
        region_level_data_dir, patient_id, "ergo2_output_with_regions.csv"
    )
    region_level_data = pd.read_csv(filepath, sep=",", comment="#")
    req_data = region_level_data[region_level_data[f"region{region}"] == "Yes"]
    req_data = pd.merge(
        req_data[["Peptide", "CDR3", "Score_mcpas"]],
        patient_data[["CDR3", "counts"]],
        on="CDR3",
        how="inner",
    )

    neoantigens = req_data["Peptide"].unique()
    num_of_neoantigens = req_data["Peptide"].nunique()
    ...
    mean_kr_and_clone_size_df = pd.DataFrame(
        columns=["peptide", "mean_kr", "mean_clone_size"]
    )  # initiate a dataframe to store final results

    for j, antigen in enumerate(neoantigens):
        if rank == 0:
            print(f"=" * 80)
            print(f"Processing neoantigen {antigen}...({j+1}/{num_of_neoantigens})\n")
            sys.stdout.flush()  # Flush the output buffer

        neoantigen_data = req_data[req_data["Peptide"] == antigen]

        # if rank == 0:
        #     print(neoantigen_data.shape)

        number_of_chunks = 5
        if neoantigen_data.shape[0] > 1000:
            chunked_patient_data = chunk_datafile(neoantigen_data, number_of_chunks)
        else:
            chunked_patient_data = chunk_datafile(neoantigen_data, 1)

        # if rank == 0:
        #     print([data_chunk.shape for data_chunk in chunked_patient_data])
        # Create a temporary folder to create meta files and later delete it
        if rank == 0:
            try:
                temp_dir = f"{root_dir}/temp_{patient_id}_region{region}_{antigen}"
                os.mkdir(temp_dir)
        ...
        for i, data_chunk in enumerate(chunked_patient_data):
            if rank == 0:
                ...
                sys.stdout.flush()  # Flush the output buffer
            scaled_kr_values = (
                data_chunk["Score_mcpas"].values * num_of_neoantigens / max_kr
            )
        ...
        if rank == 0:
            output_file = f"{root_dir}/probs_{patient_id}_region{region}_{antigen}.h5"
            ...
            # There is no need for the temporary directory now. Delete it.
            shutil.rmtree(temp_dir)
            print(f"Temporary directory {temp_dir} deleted.")
            sys.stdout.flush()

        # Now import and use the probabilities just saved and delete it later
        if rank == 0:
            probability_file = (
                f"{root_dir}/probs_{patient_id}_region{region}_{antigen}.h5"
            )
            print(
                f"Opening and generating configuration from the probability file\n{probability_file}\n"
            )
            print(
                f"=======================================================================\n"
            )
            sys.stdout.flush()
            with h5py.File(probability_file, "r") as f:
                probs = f["result"][:]
                sample_size_per_tcr = 1000
                configs = generate_configuration_per_tcr(
                    probs, sample_size_per_tcr, disable_progressbar=True
                )
                mean_kr = np.mean(
                    neoantigen_data["Score_mcpas"].values * num_of_neoantigens / max_kr
                )
                mean_clone_size = np.mean(configs.flatten())
                new_row = pd.Series(
                    {
                        "peptide": antigen,
                        "mean_kr": mean_kr,
                        "mean_clone_size": mean_clone_size,
                    }
                )
                mean_kr_and_clone_size_df = pd.concat(
                    [mean_kr_and_clone_size_df, new_row.to_frame().T], ignore_index=True
                )
            # delete the probability file for the created for the neoantigen
            shutil.os.remove(probability_file)
            print(f"The probability file {probability_file} is deleted.")
            print("Resting for 5 seconds.")
            sys.stdout.flush()
        time.sleep(5)
    # save the final_results
    if rank == 0:
        final_result_filename = f"{root_dir}/results/{patient_id}_region{region}_neoantigen_specific_mean_kr_and_clone_size_from_MCPAS.csv"
        mean_kr_and_clone_size_df.to_csv(
        ...
        print(f"Final results saved in file:\n{final_result_filename}")
        print(
            "=+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+="
        )
        sys.stdout.flush()
