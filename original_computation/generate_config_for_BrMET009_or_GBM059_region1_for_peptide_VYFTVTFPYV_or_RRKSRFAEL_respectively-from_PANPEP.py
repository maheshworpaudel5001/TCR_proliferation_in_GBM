...
if __name__ == "__main__":
    ...
    ##################Prepare data###################
    region = 1
    neoantigen_dict = {"BrMET009": "VYFTVTFPYV", "GBM059": "RRKSRFAEL"}

    full_data = pd.read_csv(
        f"{root_dir}/data/BrMET_and_GBM_data-PANPEP.csv",
        sep=",",
    )
    ...
    x1_value = patient_params_df_by_region["x1_panpep"].values[0]
    x2_value = patient_params_df_by_region["x2_panpep"].values[0]
    ...
    for i, data_chunk in enumerate(chunked_patient_data):
        if rank == 0:
            ...
            sys.stdout.flush()  # Flush the output buffer
        scaled_kr_values = data_chunk["Score"].values * num_of_neoantigens / max_kr
    ...
    if rank == 0:
        ...
        with h5py.File(probability_file, "r") as f:
            ...
            config_filename = (
                f"config_{patient_id}_region{region}_from_{antigen}_from_PANPEP.npz"
            )
